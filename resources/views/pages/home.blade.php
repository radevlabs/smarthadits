...
@section('info')
    <p>SmadHadits adalah aplikasi klasifikasi hadits berdasarkan perawi hadits. SmadHadits dapat memperkirakan keshahihan suatu hadits yang dilihat dari perawi hadits karena model kecerdasan buatan ini telah dilatih dengan 50.000 hadits lebih dengan keshahihan shahih, hasan dan dhaif. Tidak hanya itu, Smadhadits juga dapat memberikan hadits-hadits yang mirip sesuai dengan hadits yang diinputkan dan juga memberikan informasi-informasi hadits.</p>
@endsection

@section('content')
    <div class="elements-section">
        <div class="element">
            <h2 class="el-title">Hadits hari ini</h2>
            @foreach($hadith->languages()->whereIn('name', ['Indonesia', 'Arab', 'Arab (tanpa harokat)'])->get() as $language)
                <blockquote class="blockquote @if($language->name == 'Arab') text-right @endif">
                    {!! narrator(explode(':', $language->pivot->content, 2)[1]) !!}
                </blockquote>
                <p class="@if($language->name == 'Arab') text-right @endif">{{ $language->pivot->summary }}</p>
            @endforeach
